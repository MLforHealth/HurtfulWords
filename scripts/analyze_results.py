...
from sklearn.metrics import accuracy_score, roc_auc_score, average_precision_score,\
    log_loss, precision_score, confusion_matrix, recall_score
import re
...
def analyze_results(path):
    ...
    if args.bootstrap:
        for s in tqdm(range(1, args.bootstrap_samples+1)):
            ...
            calc_metrics(result_dfs, confusion_matrices, cols_in_output, df_sample, labels, prop_name, s, task_type, target, df)
    else:
        calc_metrics(result_dfs, confusion_matrices, cols_in_output, temp, labels, prop_name, 1, task_type, target, df)
    ...
    if len(confusion_matrices) > 0:
        temp = np.stack([confusion_matrices[i] for i in confusion_matrices])
        if prop_name is not None:
            temp = pd.DataFrame(np.average(temp, axis = 0), index = labels, columns = labels)
        else:
            temp = pd.DataFrame(np.average(temp, axis = 0))
        result_dfs['confusion'] = temp

    with pd.ExcelWriter(outfile_name) as writer:
        for i in result_dfs:
            if args.bootstrap and i !='confusion':
                if args.save_folds:
...
def calc_metrics(result_dfs, confusion_matrices, cols_in_output, df_fold, labels, prop_name, c, task_type, target, df):
    for g in cols_in_output:
        ...
        if task_type == 'binary':
            if g == 'all':
                calc_binary(df_fold, result_dfs[g], c, 'all', target, None, prop_name, labels)
            else:
                for j in refined_mapping:
                    q = '%s=="%s"'%(g, j)
                    calc_binary(df_fold.query(q), result_dfs[g], c, q,target, result_dfs['all'],prop_name, labels)
    ...
    if task_type == 'binary':
        confusion_matrices['confusion%s'%c] = confusion_matrix(df_fold[target], df_fold['pred'].round())


def calc_binary(temp, result_df, fold_id, prefix, target, all_df = None, prop_name = None, labels = None):
    metrics = {}
    if temp.shape[0] == 0:
        return metrics
    if len(np.unique(temp[target])) > 1:
        metrics['auroc'] = roc_auc_score(temp[target], temp['pred'])
    metrics['precision'] = precision_score(temp[target], temp['pred'].round())
    metrics['recall'] = recall_score(temp[target], temp['pred'].round())
    metrics['auprc'] = average_precision_score(temp[target], temp['pred'])
    metrics['log_loss'] = log_loss(temp[target], temp['pred'], labels = [0, 1])
    metrics['acc'] = accuracy_score(temp[target], temp['pred'].round())
    CM = confusion_matrix(temp[target], temp['pred'].round(), labels = [0, 1])
    metrics['TN'] = CM[0][0]
    metrics['FN'] = CM[1][0]
    metrics['TP'] = CM[1][1]
    metrics['FP'] = CM[0][1]
    ...
    metrics['pred_true_count'] = (temp['pred'].round() == 1).sum()
