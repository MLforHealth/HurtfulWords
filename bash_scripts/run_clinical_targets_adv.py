...
parser.add_argument("--lamb", required = True, type = float)
...
for model in std_models:
    for f in folds:
        ...
        subprocess.call(shlex.split('sbatch finetune_on_target_adv.sh "%s" "%s" "%s" "%s" "%s" "%s"'%(args.target_type, model, dev, test1, test2, args.lamb)))
